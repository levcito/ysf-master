...
namespace Natives
{
	...
	AMX_DECLARE_NATIVE(YSF_ToggleNPCOnQuery)
	{
		CHECK_PARAMS(1, LOADED);
	...
	AMX_DECLARE_NATIVE(YSF_IsNPCOnQueryEnabled)
	{
		if (!CPlugin::IsInitialized()) return std::numeric_limits<int>::lowest(); // If unknown server version
	...
	AMX_DECLARE_NATIVE(YSF_ToggleNPCOnScoreboard)
	{
		CHECK_PARAMS(1, LOADED);
	...
	// native YSF_IsNPCOnQueryEnabled();
	AMX_DECLARE_NATIVE(YSF_IsNPCOnScoreboardEnabled)
	{
		if (!CPlugin::IsInitialized()) return std::numeric_limits<int>::lowest(); // If unknown server version
...
static AMX_NATIVE_INFO native_list[] =
{
	...
	AMX_DEFINE_NATIVE(YSF_ToggleNPCOnQuery) // R20
	AMX_DEFINE_NATIVE(YSF_IsNPCOnQueryEnabled) // R20
	AMX_DEFINE_NATIVE(YSF_ToggleNPCOnScoreboard) // R20
	AMX_DEFINE_NATIVE(YSF_IsNPCOnScoreboardEnabled) // R20
	AMX_DEFINE_NATIVE(YSF_SetQueryFloodCheckType) // R20
	AMX_DEFINE_NATIVE(YSF_GetQueryFloodCheckType) // R20
};
