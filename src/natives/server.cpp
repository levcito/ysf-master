...
namespace Natives
{
	...
	AMX_DECLARE_NATIVE(SetChatTextReplacement)
	{
		CHECK_PARAMS(1, LOADED);

		CPlugin::Get()->SetChatTextReplacement(CScriptParams::Get()->ReadInt());
		return 1;
	}

	// native GetChatTextReplacement();
	AMX_DECLARE_NATIVE(GetChatTextReplacement)
	{
		return CPlugin::Get()->GetChatTextReplacement();
	}

	// native RemoveColorTags(dest[], const source[], len = sizeof(dest));
	AMX_DECLARE_NATIVE(RemoveColorTags)
	{
		CHECK_PARAMS(3, LOADED);

		std::string source;
		CScriptParams::Get()->Read(source);
		if (source.empty()) return 0;

		CAddress::FUNC_RemoveColorTags(const_cast<char*>(source.c_str()));
		CScriptParams::Get()->Add(const_cast<char*>(source.c_str()));
		return 1;
	}

	// native ReplaceBadChars(dest[], const source[], len = sizeof(dest));
	AMX_DECLARE_NATIVE(ReplaceBadChars)
	{
		CHECK_PARAMS(3, LOADED);
		...
		CAddress::FUNC_ReplaceBadChars(const_cast<char*>(source.c_str()));
		CScriptParams::Get()->Add(const_cast<char*>(source.c_str()));
		return 1;
	}

	// native ReplaceSpecialChars(dest[], const source[], len = sizeof(dest));
	AMX_DECLARE_NATIVE(ReplaceSpecialChars)
	{
		CHECK_PARAMS(3, LOADED);
		...
		for (size_t i = 0; i < strlen(const_cast<char*>(source.c_str())); i++)
		{
			if (source[i] == '\a' || source[i] == '\b' || source[i] == '\f' || source[i] == '\t' || source[i] == '\v' || source[i] == '\r')
				source[i] = '#';
		}
	...
	// native GetCurrentOS();
	AMX_DECLARE_NATIVE(GetCurrentOS)
	{
#ifdef _WIN32
		return 1;
#else
...
static AMX_NATIVE_INFO native_list[] =
{
	...
	AMX_DEFINE_NATIVE(SetChatTextReplacement) // R20
	AMX_DEFINE_NATIVE(GetChatTextReplacement) // R20
	AMX_DEFINE_NATIVE(RemoveColorTags) // R20
	AMX_DEFINE_NATIVE(ReplaceBadChars) // R20
	AMX_DEFINE_NATIVE(ReplaceSpecialChars) // R20

	AMX_DEFINE_NATIVE(GetCurrentOS) // 2.0
