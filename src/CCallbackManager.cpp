...
int CCallbackManager::OnDialogSpoof(WORD playerid, int spooftype, WORD storedid, WORD dialogid, int hits)
{
	int idx = -1;
	cell ret = -1;
	for (auto iter : m_setAMX)
	{
		if (!amx_FindPublic(iter, "OnDialogSpoof", &idx))
		{
			amx_Push(iter, static_cast<cell>(hits));
			amx_Push(iter, static_cast<cell>(dialogid));
			amx_Push(iter, static_cast<cell>(storedid));
			amx_Push(iter, static_cast<cell>(spooftype));
			amx_Push(iter, static_cast<cell>(playerid));
			amx_Exec(iter, &ret, idx);
	...
	return (ret == -1 ? ret : static_cast<int>(ret));
}
