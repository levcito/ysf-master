...
subhook_t OnDialogResponse_hook;
...
void CDECL HOOK_OnDialogResponse(RPCParameters* rpcParams)
{
	subhook_remove(OnDialogResponse_hook);

	int ret = -1;
	RakNet::BitStream bsData(rpcParams->input, rpcParams->numberOfBitsOfData / 8, false);
	int playerid = CSAMPFunctions::GetIndexFromPlayerID(rpcParams->sender);
	WORD wDialogID;
	BYTE bResponse;
	WORD wListItem;
	BYTE bTextLength;
	char szInputtext[128];

	bsData.Read(wDialogID);
	bsData.Read(bResponse);
	bsData.Read(wListItem);
	bsData.Read(bTextLength);

	if (bTextLength >= sizeof(szInputtext)) bTextLength = (BYTE)(sizeof(szInputtext) - 1);
	bsData.Read(szInputtext, bTextLength);
	szInputtext[bTextLength] = '\0';

	bsData.Reset();
	bsData.Write(wDialogID);
	bsData.Write(bResponse);
	bsData.Write(wListItem);
	bsData.Write(bTextLength);
	if (CConfig::Get()->m_bDialogProtection && IsPlayerConnected(playerid))
	{
		int hits = 0;
		auto& data = CServer::Get()->PlayerPool.Extra(playerid);
		if (data.wDialogID != wDialogID)
		{
			ret = CCallbackManager::OnDialogSpoof(playerid, 0, data.wDialogID, wDialogID, ++hits);
			if (ret == -1)
			{
				logprintf("YSF: Might dialog hack has been detected for player %s(%d) - which should be: %d, dialogid: %d", GetPlayerName(playerid), playerid, data.wDialogID, wDialogID);
				ret = 0; // default action: block spoofed dialog.
			}
		}

		for (size_t i = 0; i < strlen(szInputtext); i++)
		{
			if (szInputtext[i] == '%')
			{
				ret = CCallbackManager::OnDialogSpoof(playerid, 1, data.wDialogID, wDialogID, ++hits);
				if (ret == 2) szInputtext[i] = '#';
				else if (ret == -1) logprintf("YSF: Dialog format specifier input has been detected for player %s(%d), dialogid: %d", GetPlayerName(playerid), playerid, wDialogID);
			}
			else if (szInputtext[i] == '\n')
			{
				ret = CCallbackManager::OnDialogSpoof(playerid, 2, data.wDialogID, wDialogID, ++hits);
				if (ret == 2) szInputtext[i] = '\0';
				else if (ret == -1) logprintf("YSF: Dialog new line input has been detected for player %s(%d), dialogid: %d", GetPlayerName(playerid), playerid, wDialogID);
			}
		}

		data.wDialogID = -1;
	}
	bsData.Write(szInputtext, bTextLength);

	if (ret != 0) CAddress::FUNC_OnDialogResponse(rpcParams);

	subhook_install(OnDialogResponse_hook);
}
...
{
	switch (CPlugin::Get()->GetChatTextReplacement())
	{
		//case 0: // disable
		case 1: // normal
			...
			break;
		case 2: // fix
			ReplaceBadCharsFix(szString);
			break;
		case 3: // fix (allow colors)
			ReplaceBadCharsFix(szString, false);
			break;
	}
}
...
void InstallPreHooks()
{
	...
	OnDialogResponse_hook = HOOK(OnDialogResponse);
...
void UninstallHooks()
{
	...
	SUBHOOK_REMOVE(ReplaceBadChars_hook);
	SUBHOOK_REMOVE(OnDialogResponse_hook);
	SUBHOOK_REMOVE(CGameMode__OnPlayerConnect_hook);
	SUBHOOK_REMOVE(CGameMode__OnPlayerDisconnect_hook);
	SUBHOOK_REMOVE(CGameMode__OnPlayerSpawn_hook);
