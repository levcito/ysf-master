...
{
    SendClientMessagef(playerid, 0x33FF33AA, "Kurva %s %d %.3f", "anyd", -1547, 2654.6542);
    SendClientMessageToAllf(0x33FF33AA, "lofasz a %s", "fledbe");

    GameTextForPlayerf(playerid, 3000, 3, "Kis %s", "kecske");
    GameTextForAllf(3000, 3, "Kis %s", "kecske");

    SendPlayerMessageToPlayerf(playerid, playerid, "megbasztam %s %s htn", "apdat", "anyd");
    SendPlayerMessageToAllf(playerid, "megbasztam %s %s htn", "apdat", "anyd");
    return 1;
}
...

{
    ...
    printf("lfasz - %d", limitglobalchat);
}
/*
...
public OnDialogSpoof(playerid, spooftype, storedid, dialogid, hits)
{
	new szSpoofType[3][] =
    {
        "spoofed dialogid",
	...
	SendClientMessagef(playerid, -1, "* Dialog blocked due to %s.",  szSpoofType[spooftype == -1 ? 0 : spooftype]);
	return 0;
}
